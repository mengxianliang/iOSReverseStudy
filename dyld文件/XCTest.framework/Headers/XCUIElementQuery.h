#if __has_include(<XCUIAutomation/XCUIElementQuery.h>)
...
+ (instancetype)new XCT_UNAVAILABLE("Use XCUIElementTypeQueryProvider methods on XCUIApplication or an existing XCUIElementQuery to create XCUIElementQuery instances.");
- (instancetype)init XCT_UNAVAILABLE("Use XCUIElementTypeQueryProvider methods on XCUIApplication or an existing XCUIElementQuery to create XCUIElementQuery instances.");
...
- (XCUIElement *)elementAtIndex:(NSUInteger)index
    XCT_DEPRECATED_WITH_DIRECT_REPLACEMENT("elementBoundByIndex:")
    XCT_DEPRECATED_WITH_SWIFT_REPLACEMENT("element(boundBy:)");
